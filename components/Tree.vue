...
<template>
  <svg :x="x" :y="y" style="overflow:visible;">
    <Cell :vueblank="vueblank" :binarr="BinaryArray()" :dx="0" :dy="0" />
...
export default {
  ...
  props: ['x', 'y', 'x1', 'x2', 'y1', 'y2', 'vueblank', 'display'],
  methods: {
    // Top-2-Bottom
    BinaryArray: function(for1, for2) {
      if (this.display > 3) {
        /* eslint-disable no-console */
        console.log('tree cant take display>3')
      }
      // NOTE: this controles bottom dots for non-all displays
      const [dot3, dot6] = [[0, 0], [0, 0], [1, 0], [1, 1]][this.display]
      if (typeof for1 === 'undefined') {
        // First cell
        const cell1 = [0, 0, dot3, 0, 0, dot6]
        const numsign = [0, 0, 1, 1, 1, 1]
        return +this.display === 0 ? numsign : cell1
